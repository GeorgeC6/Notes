...
$$
\widehat{H} = \hbar \sum_{j = 1,2} -\frac{1}{2} \hbar \omega_j \hat{\sigma}_z^j + \hbar \textcolor{blue}{g} (\sigma_1^+ \sigma_2^- + \sigma_1^- \sigma_2^+)
$$

谐振腔耦合

$$
\widehat{H} = \hbar \omega_R \hat{a}^\dagger \hat{a} + \hbar \sum_{j = 1,2} \left[-\frac{1}{2} \omega_j \hat{\sigma}_z^j + \textcolor{blue}{g_j} (\hat{a}^\dagger \sigma_j^- + \hat{a} \sigma_j^+)\right]
$$
